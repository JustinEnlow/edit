...
//
    ...
    match crate::utilities::extend_selection_line_text_start::selection_impl(selection, buffer, semantics.clone()){
        Err(_) => crate::utilities::extend_selection_line_start::selection_impl(selection, buffer, semantics),
        Ok(new_selection) => Ok(new_selection)
    }
}

...
/// # Errors
/// when no matches.
pub fn incremental_search_in_selection(
    selections: &Selections, 
    input: &str, 
    buffer: &Buffer, 
    semantics: CursorSemantics
    ...
    for selection in &selections.flatten(){  //self.selections.iter(){   //change suggested by clippy lint
        ...
        let matches = crate::selection::incremental_search_in_selection(selection, input, buffer);
        if selection == primary_selection{
            primary_selection_index = num_pushed.saturating_sub(1);
        }
...
pub fn incremental_split_in_selection(
    selections: &Selections, 
    input: &str, 
    buffer: &Buffer, 
    semantics: CursorSemantics
    ...
    for selection in &selections.flatten(){
        //let matches = selection_impl(selection, input, buffer);
        let matches = crate::selection::incremental_split_in_selection(selection, input, buffer);
        if matches.is_empty(){
